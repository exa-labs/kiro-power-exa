...
- **`web_search_advanced_exa`**: same backend with explicit filters (`includeDomains`, `excludeDomains`, `startPublishedDate`, `endPublishedDate`, `category`, `userLocation`), plus configurable summaries, highlights, and subpage crawling. Reach for this when you need domain or date constraints that `category:` alone can't express.
